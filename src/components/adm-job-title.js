...
const AdmJobTitle = (props) => {
  ...
  const fetchJobTitle = async () => {
    ...
    const response = await fetch("http://localhost:8000/api/classifier/get_all_job_title", requestOptions)
  ...
  const createField = async () => {
    ...
    const response = await fetch("http://localhost:8000/api/classifier/create_job_title", requestOptions)
  ...
  const updateField = async () => {
    ...
    var response = await fetch(`http://localhost:8000/api/classifier/update_job_title/${choose}`, requestOptions)
    var fetchInfo = await response.json();
    if (response.ok)
    {
  ...
  const deleteField = async () => {
    ...
    var response = await fetch(`http://localhost:8000/api/classifier/delete_job_title/${choose}`, requestOptions)
    var fetchInfo = await response.json();
    if (response.ok)
    {
    ...
    <div className={`adm-job-title-container1 ${props.rootClassName} `}>
      ...
      <div className="adm-job-title-container2">
        <div className="adm-job-title-container3">
          ...
          {filteredItems.map((item) => (
            <ElemName setChoose={setChoose}
              ...
              name={
                <Fragment>
                  <span className="adm-job-title-text15">{item["name"]}</span>
                </Fragment>
              }
        ...
        <div className="adm-job-title-container6">
          ...
          {choose == 0 ?
          <button type="button" onClick={createField} className="adm-job-title-button3 button">
            <span>Создать</span>
          </button>
          :
          <>
          <button type="button" onClick={updateField} className="adm-job-title-button4 button">
            <span>Изменить</span>
          </button>
          <button type="button" onClick={deleteField} className="adm-job-title-button5 button">
            <span>Удалить</span>
          </button>
          </>
          }
          <span id="error" className="adm-job-title-text20"></span>
        </div>
      </div>
    </div>
  )
